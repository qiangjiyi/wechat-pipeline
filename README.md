...
从原始内容出发，依次完成格式化、原生视觉 Skill 执行、gzh-design 公众号排版、产物验收，并安全写入微信公众号草稿箱。
...
- **一次请求，一个运行目录**：格式化、原生 Skill 自然产物、manifest 和发布适配文件都属于同一个 `run_id`。
- **Native Skill First**：Designer 只触发完整 Baoyu Skill，不规划、复述或重建 Skill 内部流程。
- **确定性控制面**：每次状态转换自动执行对应门禁，Agent 不能靠文字回报推进流程。
- **冻结发布快照**：Publisher 只消费绑定 source、content、图片、HTML 和 validator hash 的只读快照。
- **隔离自然产物**：每个视觉 Skill 直接在运行目录下的同名 workspace 内自由产出，并服从自己的 `EXTEND.md` 输出目录偏好；`.pipeline/` 只保存调用边界与发布证据。
...
| 内容格式化 | 结构校验 / `baoyu-format-markdown` | 结构校验 / `baoyu-format-markdown` |
| 图片生成 | `baoyu-xhs-images` | 封面 + 文章内联图 |
| 公众号排版 | 不适用 | `gzh-design` 主题组件库 + HTML 校验 |
| 图片 Provider fallback | 由原生 Skill 决定 | 由原生 Skill 决定 |
| 原生 Skill 完成验收 | 支持 | 支持 |
| 多公众号账号 | 支持 | 支持 |
...
- `wechat-pipeline`：Claude Code 与 Codex 共用的总入口和唯一协调器
...
视觉 Skill 按各自当前说明和 EXTEND 自主选择宿主原生图片能力或 `baoyu-image-gen` Provider。相关配置位于：

```text
~/.baoyu-skills/.env
```

Pipeline 不覆盖视觉 Skill 的 backend、batch 或 fallback 策略，也不读取和输出 API Key。
...
完整流水线每次都会调用原生 Formatter；已有 Markdown 结构时由 Formatter Skill 自然决定保持不变或只做少量调整。统一的 Skill 边界会先验证 `baoyu-format-markdown/article-formatted.md` 保留全部源文，再生成只读 `content.md`。News 的封面与正文配图在两个隔离 Worker 中并行，全部通过门禁后 Typesetter 才启动。Skill 返回最终 HTML 后，由确定性脚本生成 canonical layout 产物并验收；Publisher 只能发布随后冻结的快照。
...
flowchart LR
    ...
    R --> F["Formatter / 结构校验"]
    F --> C["冻结 content.md"]
    C --> X["newspic：贴图 Skill"]
    C --> CV["news：封面 Skill"]
    C --> IV["news：正文配图 Skill"]
    X --> V2["Skill 完成、源文与 role 门禁"]
    CV --> V2
    IV --> V2
    V2 --> T["Typesetter 自然执行 gzh-design（仅 news）"]
    T --> Q["脚本封装 canonical layout 并最终验收"]
    Q --> LV["Layout HTML 校验"]
    V2 --> S["冻结 Publish Snapshot（newspic）"]
    LV --> S["冻结 Publish Snapshot（news）"]
    S --> P2["Publisher 上传图片并替换 src"]
    P2 --> W["创建唯一草稿并原子写回执"]
    W --> RV["draft/get 回读验收"]
    RV --> DONE["published"]
```

Claude Code 与 Codex 都由顶层 `wechat-pipeline` Skill 作为唯一 Leader，直接调度 Formatter、Designer、Typesetter 和 Publisher。入口不会先创建中间 Leader Agent，因此所有 Worker 都是第一层子 Agent；两种宿主共享同一份运行协议和校验脚本。

状态只能按实际阶段流转：

```text
newspic: input_sealed -> formatting -> content_ready -> designing -> artwork_ready -> publish_ready -> publishing -> published
news:    input_sealed -> formatting -> content_ready -> designing -> artwork_ready -> typesetting -> layout_ready -> publish_ready -> publishing -> published
```
...
└── wechat-pipeline/
    └── <slug>-<run_id>/
        ...
        ├── gzh-design/
        └── .pipeline/
            ├── layout-skill-run.json
            ├── layout.json
            ├── layout-validation.json
            └── publish-snapshot.json
```

`.pipeline/` 保存审计与验收元数据；运行根目录下的 Skill 同名目录原样保留其自然产物。未修改的执行期 `article.md` 输入副本会在成功后清理。
...
│   ├── agents/                     # 4 个 Claude Code Worker Agents
...
- Doctor 检查本地配置、运行包完整性和图片 API dialect，不打印密钥，也不在线验证 token。
- 默认只创建草稿，不执行正式群发。
- 命名账号凭据严格隔离，不回退到全局账号。
...
- 长耗时 worker 通过 `progress.json` 报告结构化进度；视觉门禁绑定原生 Skill 的完整执行和最终返回结果，仅额外执行 news 封面 `2.35:1` 的发布合同，不二次定义其他图片尺寸或像素质量。
- Agent 不得绕过 manifest 校验补文件、篡改 Skill 已使用的 prompt、重命名图片或自行伪造 Skill 产物。
