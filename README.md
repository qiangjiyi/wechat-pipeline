...
如果输入已经具有可用的 Markdown 标题结构，流水线会跳过 LLM Formatter，但仍生成并验证只读 `content.md`。Designer 只读取这份 canonical 内容；所有图片完成并通过门禁后，Typesetter 才启动。V2 只在同一 Designer 内并行生成彼此独立的图片，不再让 Designer 与 Typesetter 同时修改共享产物。排版完成后生成只读 publish snapshot，Publisher 只能发布快照绑定的内容。
...
flowchart LR
    ...
    D --> F["Formatter / 结构校验"]
    F --> C["冻结 content.md"]
    C --> P1["Designer 规划 prompt"]
    P1 --> V1["Plan 校验"]
    V1 --> G["执行 Baoyu Skill 生图"]
    G --> V2["Artwork 门禁"]
    V2 --> T["Typesetter 执行 gzh-design（仅 news）"]
    T --> LV["Layout HTML 校验"]
...
```text
newspic: input_sealed -> formatting -> content_ready -> planning -> rendering -> artwork_ready -> publish_ready -> publishing -> published
news:    input_sealed -> formatting -> content_ready -> planning -> rendering -> artwork_ready -> typesetting -> layout_ready -> publish_ready -> publishing -> published
```
