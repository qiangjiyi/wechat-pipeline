...
- Claude Code Agent 必须通过宿主 Agent 工具按声明名派发；`agents/*.md` 不是可执行 Python。禁止尝试 `agents/*/agent.py`，禁止 Leader 在派工失败后冒充 Worker。
...
协议升版必须同步 `protocol_version.py`、5 个 Agent、2 个入口 Skill 和本文件，共 9 处；结构测试负责看守。
