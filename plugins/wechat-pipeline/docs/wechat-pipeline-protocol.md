...
- News Typesetter 开始前设为 `typesetting`，HTML 与 layout manifest 验收通过后设为 `layout_ready`。
...
```bash
python3 "${PIPELINE_ROOT}/scripts/prepare_article_source.py" \
...
Newspic Publisher 继续直接消费 Baoyu 原生图片路径。News Publisher 必须消费已验收的 `article-body.html`，不得再次把 Markdown 渲染为 HTML：

- 调用 `publish.py article --html <run-dir>/article-body.html --layout-manifest <run-dir>/.pipeline/layout.json`；
...
- 提交前必须同时通过 designer `publish-ready` 与 layout 验收。

Publisher 只能替换图片 `src`，不得修改其他正文、重新排版、重新生图或补 manifest。旧 Markdown renderer 仅保留为显式兼容路径，不得在同一 run 中静默 fallback；草稿 API 一旦成功不得创建第二份草稿。
