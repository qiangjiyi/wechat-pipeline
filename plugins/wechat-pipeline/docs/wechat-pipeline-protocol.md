...
`EXTEND.md` 由 Designer 用确定性脚本解析，**不依赖 Skill 内部的三级路径查找**（那套查找含 shell 参数展开 `${XDG_CONFIG_HOME:-$HOME/.config}`，交给 LLM 不可靠）：

```bash
"${PIPELINE_ROOT}/scripts/run_python.sh" "${PIPELINE_ROOT}/scripts/load_extend.py" <skill-name> --base-dir <run-dir> --json
```

脚本按 project -> XDG -> user home 三级顺序首个命中即返回绝对路径与 sha256；`baoyu-image-gen` 额外检查 legacy `baoyu-imagine` 路径。命中时 Designer 直接读取该绝对路径。非交互模式下未命中**不是错误**：回退 Skill 内置默认值，`preferences.source=auto`，不记 `extend_path`，继续生成。
...
| 输入、Skill 或运行上下文缺失；或 `source=extend` 但 EXTEND.md 实际丢失 | `contract_error` | 停止并报告 Leader |

非交互模式下 EXTEND.md 未命中**不是** `contract_error`：Designer 用 `load_extend.py` 解析未命中时回退 Skill 内置默认值，`preferences.source=auto`，继续生成。只有当 manifest 声明 `source=extend` 却找不到对应 EXTEND.md 时，才算 `contract_error`。
...
`preferences.source` 为 `auto` 时 `extend_path`/`extend_sha256` 必须留空（null 或省略）；仅当 `load_extend.py` 命中并实际读取了 EXTEND.md 时才记 `source=extend` 及其绝对路径与 sha256。校验器对 `source=extend` 要求 `extend_path` 非空且 hash 匹配，对 `source=auto` 不要求 `extend_path`。
