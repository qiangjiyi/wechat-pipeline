...
2. 完整读取 `<PIPELINE_ROOT>/docs/wechat-pipeline-protocol.md`，要求 `protocol_version: 2026-07-21-001`。
3. 若 source 是聊天正文，入口主线程先用宿主 Write 能力写入权限 `0600` 的临时 Markdown，派工时只传文件绝对路径，并在 init 尝试结束后的 `finally` 删除临时文件。Worker 不负责原稿落盘。
...
5. 当前入口主线程是唯一 Leader：Claude Code 设置 `HOST_RUNTIME=claude-code`，Codex 设置 `HOST_RUNTIME=codex`。禁止再派发 `wechat-leader` 或任何中间协调 Agent；所有 Formatter、Designer、Typesetter、Publisher 都必须由入口主线程直接派发，避免子 Agent 再派子 Agent。
6. Claude Code 派工时 `subagent_type` 先使用声明短名（`wechat-formatter`、`wechat-designer`、`wechat-typesetter`、`wechat-publisher`）；宿主返回 not found 时立即使用插件全限定名（如 `wechat-pipeline:wechat-typesetter`）重试。Codex 派工使用宿主 subagent 工具，视觉 Worker 使用 `fork_turns: "none"`。工具不可用时返回 `blocked`，不能在 Leader 上下文代做。
...
2. 推进 `designing`。newspic 启动并派发 `baoyu-xhs-images`。news 先分别 start `baoyu-cover-image` 与 `baoyu-article-illustrator`，随后同时派两个互不继承上下文的 Designer：
...
3. 进入 `designing`：Leader 为模式要求的每个原生视觉 Skill 在运行根目录创建同名独立 workspace 和调用记录。news 的 cover 与 article-illustrator 同时派两个全新 Designer Worker；二者没有先后依赖。两个宿主都使用独立 Agent invocation；Codex 使用 `fork_turns: "none"`，不继承 Leader 或另一个视觉 Worker 的会话。
