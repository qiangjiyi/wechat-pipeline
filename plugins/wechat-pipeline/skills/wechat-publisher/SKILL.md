...
把已经完成的图片卡片或已排版文章推送到微信公众号草稿箱，不做正式群发。流水线模式要求 `protocol_version: 2026-07-20-001`。
...
5. 流水线模式先运行 `${PIPELINE_ROOT}/scripts/run_context.py guard <run-dir> publisher`，再运行 `${PIPELINE_ROOT}/scripts/build_publish_snapshot.py <run-dir> --validate`，只接受 canonical `.pipeline/publish-snapshot.json`。`publish_ready` 只允许 dry-run，真实网络发布必须处于 `publishing`。
6. `article --html` 还必须提供 `--layout-manifest`；newspic 必须提供 `--manifest`。两种模式都传 `--snapshot`、固定写回执并回读验证。恢复时复用素材上传检查点；若同一请求指纹已经有 `draft_media_id`，只重试回读验证。`draft/add` 响应不确定时停止并保留 `creation_status: unknown`，不再次创建草稿；人工确认后使用 `--recover-draft-media-id` 绑定并验证。
7. 运行 dry-run 或展示确认信息。除非用户明确要求 `--yes`，发布前必须确认。
8. 使用完整绝对路径调用：
...
9. 成功时回报账号、模式、`draft_media_id` 和回读验证；失败时回报脚本的真实错误及已落盘回执。

## 重试语义

Publisher 对可安全重试的读取与素材上传，在连接错误、超时、HTTP 408/429/5xx 时执行 `30/60/120` 秒退避。非幂等 `draft/add` 不自动重试；微信业务 `errcode` 不重试。代码完成退避后仍失败时，不要再次从 Agent 层循环调用整个发布流程。
...
- Python 3.10+；通过 Plugin 的 `run_python.sh` 自动选择，也可用进程变量 `WECHAT_PIPELINE_PYTHON` 显式指定
