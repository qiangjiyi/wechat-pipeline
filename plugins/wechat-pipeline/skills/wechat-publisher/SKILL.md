...
6. 流水线 newspic 固定使用 `--manifest <run-dir>/.pipeline/manifest.json --result-output <run-dir>/.pipeline/publish-result.json --verify-draft`；news 同样固定写回执并回读验证。恢复时复用素材上传检查点；若同一请求指纹已经有 `draft_media_id`，只重试回读验证。`draft/add` 响应不确定时停止并保留 `creation_status: unknown`，不再次创建草稿；人工确认后使用 `--recover-draft-media-id` 绑定并验证。
