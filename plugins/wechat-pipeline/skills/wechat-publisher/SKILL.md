...
description: 把已经做好的内容发布到微信公众号草稿箱。支持微信贴图（article_type=newspic，短文 + 1-20 张图片）和微信公众号文章（article_type=news，Markdown 渲染 HTML）。当用户说「微信贴图一键发布」「把图片卡片发公众号」「发到公众号草稿箱」「newspic 草稿」「发布公众号文章」「把 Markdown 发公众号」时使用。只负责发布，不生成图片。
---

# 微信公众号发布

把已经完成的图片卡片或 Markdown 文章推送到微信公众号草稿箱，不做正式群发。
...
- `article`：Markdown 渲染 HTML，上传正文图片和封面，发布为微信文章。
...
5. 运行 dry-run 或展示确认信息。除非用户明确要求 `--yes`，发布前必须确认。
6. 使用完整绝对路径调用：
...
7. 成功时回报账号、模式和 `draft_media_id`；失败时回报脚本的真实错误。
...
- Python 3.10+
- article 模式需要 Node.js + npm
- article 首次运行会把 npm 依赖安装到 `${CLAUDE_PLUGIN_DATA}/wechat-publisher/`；其他宿主使用 `~/.cache/wechat-pipeline/wechat-publisher/`
