...
8. 首轮只完成 prompt 与 manifest 规划并回报 Leader；不得自行推进 run 状态。Leader 完成 `--phase plan` 校验并把状态设为 `rendering` 后，恢复同一个 Designer 才能开始真实生图。
9. 全部图片生成后更新真实 attempts、输出路径和 hash，再回报 Leader；由 Leader 执行 `--phase publish-ready` 并把状态设为 `ready`。
...
回报必须包含 `protocol_version`、`run_id`、canonical 目录、读取的 Skill/reference/EXTEND、偏好来源、provider 链、manifest 路径和当前阶段。worker 不得调用 `run_context.py status`；失败时只报告真实错误，由 Leader 写入 `failed` 状态。
