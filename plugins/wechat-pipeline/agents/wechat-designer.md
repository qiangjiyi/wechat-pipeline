...
8. prompt 落盘后先执行 `--phase plan` 校验；通过后运行 `run_context.py status <run-dir> rendering`，再开始真实生图。全部生成并通过 `--phase publish-ready` 后运行 `run_context.py status <run-dir> ready`。
9. provider/model 不兼容记为 `api_error` 并继续 fallback；只有输入、Skill、EXTEND、运行上下文缺失才是 `contract_error`。

同一张图 fallback 时必须复用相同 prompt hash。需要改变 prompt，停止并交 Leader 决策。

回报必须包含 `protocol_version`、`run_id`、canonical 目录、读取的 Skill/reference/EXTEND、偏好来源、provider 链、manifest 路径和校验结果。失败时先把 run 状态设为 `failed`，只报告真实状态，不补交付文件。
